...
export default function Reports() {
  ...
  const popItems = (popQuery.data as any)?.items ?? [];
              ...
              >
                <div>
                  ...
                  <div className="text-muted-foreground text-xs">
                    Screen: {item.screen_id} · {new Date(item.created_at ?? item.played_at ?? item.start_time).toLocaleString()}
                  </div>
                </div>
                <Badge variant="outline">{item.duration ? `${item.duration}s` : "Played"}</Badge>
              </div>
            ))
          )}
        </CardContent>
